...
public class RandMinionBoss : MonoBehaviour
{
    ...
    public int minionCount = 3; // Number of minions to spawn
    public float minDistance = 1f; // Minimum distance between minions
    public Vector3 xRange = new Vector3(459, 480, 0); // X range (min, max, unused)
    public Vector3 zRange = new Vector3(399, 414, 0); // Z range (min, max, unused)
    public float yPosition = 17; // Fixed Y position
    

    public int maxAggressiveMinions = 3; // Maximum number of minions that can attack at once
    ...
    public void SpawnMinions()
    {
        ...
        while (spawned < minionCount)
        {
            ...
            if (IsPositionValid(randomPosition))
            {
                ...
                MinionHealthbarBoss healthBarScript = minion.GetComponent<MinionHealthbarBoss>();
                if (healthBarScript != null)
                {
                    healthBarScript.healthBarParent = healthBar.transform; // Pass the health bar parent
                }
                minion.GetComponent<MinionBehaviorBoss>().Initialize(this); // Initialize MinionBehaviorBoss
                minions.Add(minion);
    ...
    void HandleMinionBehavior()
    {
        if (player != null && IsPlayerInRange()) // Only handle minions if player is within range
        {
            ...
            {
                MinionBehaviorBoss behavior = minion.GetComponent<MinionBehaviorBoss>();
